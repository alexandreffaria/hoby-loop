...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (!user.is_active) {
      alert('Your admin account has been deactivated. Please contact the system administrator.')
  ...
  return (
    <div className="max-w-4xl mx-auto min-h-screen p-5 font-sans text-main-text">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-black uppercase bg-gradient-secondary-tertiary text-transparent bg-clip-text">Admin Dashboard</h1>
          <p className="text-xs text-gray-400">Welcome, {user?.name}</p>
        </div>
        
        <div className="flex gap-3">
          <button onClick={() => navigate('/config')} className="text-2xl" title="Settings">⚙️</button>
          <button onClick={logout} className="text-xs text-red-400 font-bold underline self-center">Logout</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="p-1 rounded-2xl bg-gradient-secondary-tertiary">
          <div className="bg-background p-4 rounded-xl">
            <h2 className="text-sm font-bold text-gray-400 uppercase">Users</h2>
            <p className="text-3xl font-black">{users.length}</p>
            <div className="mt-2 text-xs text-gray-500">
              <span className="mr-2">Sellers: {getUserCounts().seller}</span>
              <span className="mr-2">Consumers: {getUserCounts().consumer}</span>
              <span>Admins: {getUserCounts().admin}</span>
            </div>
          </div>
        </div>
        <div className="p-1 rounded-2xl bg-gradient-tertiary-forth">
          <div className="bg-background p-4 rounded-xl">
            <h2 className="text-sm font-bold text-gray-400 uppercase">Subscriptions</h2>
            <p className="text-3xl font-black">{subscriptions.length}</p>
          </div>
        </div>
        <div className="p-1 rounded-2xl bg-gradient-secondary-forth">
          <div className="bg-background p-4 rounded-xl">
            <h2 className="text-sm font-bold text-gray-400 uppercase">Baskets</h2>
      ...
      <div className="flex bg-background rounded-lg p-1 mb-6 border border-gray-800">
        <button
          ...
          className={`flex-1 px-3 py-2 text-xs font-bold rounded-md transition-all duration-300 ${activeTab === 'users' ? 'bg-gradient-secondary-tertiary text-white' : 'text-gray-400'}`}
        >
          USERS
        </button>
        <button
          onClick={() => setActiveTab('subscriptions')}
          className={`flex-1 px-3 py-2 text-xs font-bold rounded-md transition-all duration-300 ${activeTab === 'subscriptions' ? 'bg-gradient-secondary-tertiary text-white' : 'text-gray-400'}`}
        >
          SUBSCRIPTIONS
        </button>
        <button
          onClick={() => setActiveTab('baskets')}
          className={`flex-1 px-3 py-2 text-xs font-bold rounded-md transition-all duration-300 ${activeTab === 'baskets' ? 'bg-gradient-secondary-tertiary text-white' : 'text-gray-400'}`}
        >
          BASKETS
        </button>
      </div>
      ...
      {!isLoading && activeTab === 'users' && (
        <div className="bg-background p-5 rounded-2xl border border-gray-800">
      ...
      {!isLoading && activeTab === 'subscriptions' && (
        <div className="space-y-4">
          {subscriptions.length === 0 && <p className="text-center text-gray-400 text-sm">No active subscriptions.</p>}
          {subscriptions.map(sub => (
            <div key={sub.ID} className="p-1 rounded-2xl bg-gradient-tertiary-forth">
              <div className="bg-background p-4 rounded-xl flex justify-between items-start">
                <div>
                  <h2 className="text-lg font-black uppercase text-main-text">{sub.user?.name || 'User'}</h2>
                  <p className="text-sm text-gray-400">Basket: {sub.basket?.name || 'Unknown'}</p>
      ...
      {!isLoading && activeTab === 'baskets' && (
        <div className="space-y-4">
          {baskets.length === 0 && <p className="text-center text-gray-400 text-sm">No products available.</p>}
          {baskets.map(basket => (
            <div key={basket.ID} className="p-1 rounded-2xl bg-gradient-secondary-forth">
              <div className="bg-background p-4 rounded-xl flex justify-between items-start">
                ...
                <div className="text-right">
                  ...
                  <span className="text-xs text-gray-500">Seller ID: {basket.seller_id || basket.UserID}</span>
                </div>
              </div>
            </div>
          ))}
